...
class TestSimulationFlow:
    ...
    def setup_method(self):
        # We need to recreate TestClient for each setup to ensure app state isolation
        # if possible, but in-memory repo usually persists if module-global.
        # However, for API testing, it's fine.
        self.client = TestClient(app)
        # Add default auth header
        self.auth_headers = {"Authorization": "Bearer test-user"}
    ...
    def test_simulation_workflow(self):
        ...
        create_resp = self.client.post(
            "/api/v1/sprites", json=sprite_payload, headers=self.auth_headers
        )
        ...
        list_resp = self.client.get("/api/v1/sprites", headers=self.auth_headers)
        ...
        for s in sprites_list:
            if s["id"] == sprite_id:
                found = True
                # Check if versions are populated (depends on list implementation depth)
                if "versions" in s:
        ...
        detail_resp = self.client.get(
            f"/api/v1/sprites/{sprite_id}", headers=self.auth_headers
        )
        ...
        if image_url.startswith("/"):
            ...
            img_resp = self.client.get(image_url, headers=self.auth_headers)
            # Depending on storage backend (FileSystem), it might 404 if the
            # path isn't mounted in TestClient. But usually standard static mounts work.
